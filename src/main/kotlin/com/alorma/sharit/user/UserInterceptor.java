...
public final class UserInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		if (SecurityContext.INSTANCE.userSignedIn() || requestForSignIn(request)) {
			return true;
		} else {
	...
	public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex) throws Exception {
		SecurityContext.INSTANCE.remove();
	}
	...
	private void rememberUser(HttpServletRequest request, HttpServletResponse response) {
		...
		SecurityContext.INSTANCE.setUser(new User(userId));
	}

	private void handleSignOut(HttpServletRequest request, HttpServletResponse response) {
		if (SecurityContext.INSTANCE.userSignedIn() && request.getServletPath().startsWith("/signout")) {
			String id = SecurityContext.INSTANCE.getUser().getId();
			connectionRepository.createConnectionRepository(id).removeConnections("facebook");
			userCookieGenerator.removeCookie(response);
			SecurityContext.INSTANCE.remove();
		}
	}
